...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create finlab
  fly apps create finlab-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app finlab-template
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app finlab-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'finlab-staging' does not match app name in config file 'finlab'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name finlab-db
  fly postgres attach --app finlab finlab-db

  fly postgres create --name finlab-staging-db
  fly postgres attach --app finlab-staging finlab-staging-db
  ```
